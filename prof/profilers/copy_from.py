...
@time
@sizecheck
def ohio_pg_copy_from_10_10(engine, query):
    """pg_copy_from(buffer_size=10, iter_size=10) {COPY → PipeTextIO → pandas.read_csv}"""
    return pandas.DataFrame.pg_copy_from(query,
                                         engine,
                                         parse_dates=['as_of_date'],
                                         buffer_size=10,
...
@time
@sizecheck
def ohio_pg_copy_from_10_10000(engine, query):
    """pg_copy_from(buffer_size=10, iter_size=10_000) {COPY → PipeTextIO → pandas.read_csv}"""
    return pandas.DataFrame.pg_copy_from(query,
                                         engine,
                                         parse_dates=['as_of_date'],
                                         buffer_size=10,
